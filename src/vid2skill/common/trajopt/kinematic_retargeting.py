...
class SolverConfig:
    ...
    major_iteration_limit: int = 5000
    minor_iteration_limit: int = 20000
    ...
    def get_solver_options(self) -> Dict[str, any]:
        ...
        return {
            ...
            "Major iteration limit": self.major_iteration_limit,
            "Minor iteration limit": self.minor_iteration_limit,
        }
...
class HandKeypointScaler:
    ...
    def scale_keypoints(
        self, ref_hand_keypoints_pos: np.ndarray, optimization: "OptimizationProblem"
    ) -> np.ndarray:
            ...
            for j in range(
                ...
                if j == 1 + i * n_joints_per_finger:
                    scaled_hand_keypoints_pos[j] = (
                        ref_hand_keypoints_pos[j] - wrist_pos
                    ) + wrist_pos
                else:
                    scaled_hand_keypoints_pos[j] = (
...
class OptimizationProblem:
    ...
    def add_no_penetration_constraints(self):
        min_dist_constraint = MinimumDistanceLowerBoundConstraint(
            self.drake_system.plant,
            -0.017,
            self.drake_system.plant_context,
            influence_distance_offset=1e-2,
        )
    ...
    def add_tracking_cost(
        self,
        tracking_weights: TrackingWeights,
        scaled_ref_hand_keypoints_pos: np.ndarray,
    ):
        ...
        def tracking_cost_evaluator(vars):
            ...
            for i in range(len(cur_hand_keypoints_pos)):
                tracking_error = (
                    cur_hand_keypoints_pos[i] - scaled_ref_hand_keypoints_pos[i]
                )
...
class KinematicRetargeting:
    ...
    def __init__(
        ...
        self.optimization = OptimizationProblem(
            drake_system, self.n_q, self.solver_config
        )
        ...
        self.scaled_ref_hand_keypoints_pos = self.keypoint_scaler.scale_keypoints(
            ref_hand_keypoints_pos, self.optimization
        )

        # Add constraints and costs
        self.optimization.add_tracking_cost(
            self.tracking_weights, self.scaled_ref_hand_keypoints_pos
        )
        self.optimization.add_object_pose_constraint(self.ref_obj_pose)
        self.optimization.add_joint_limit_constraints()
        self.optimization.add_no_penetration_constraints()
